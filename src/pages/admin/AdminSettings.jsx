...
import { Save, Settings, RefreshCw, CheckCircle2, AlertCircle, Clock3, FileText, Mail as MailIcon, Send, ShieldCheck, ExternalLink, Download } from 'lucide-react';
...
const PWA_SETTING_FIELDS = [
    { key: 'pwa.install_prompt_interval_days', label: 'Intervalo do lembrete de instalação', description: 'Depois que o usuário fechar o aviso, defina em quantos dias ele poderá aparecer novamente. O valor mínimo é 1 dia.', type: 'number', min: 1, max: 365 },
];
...
export default function AdminSettings() {
    ...
    return <div className="space-y-6">
        ...
        <section className="rounded-3xl bg-white border border-[#EADFD8] shadow-warm p-5 sm:p-6"><div className="flex items-start gap-3 mb-5"><div className="w-11 h-11 rounded-2xl bg-[#E9F6EE] text-[#2B7A48] flex items-center justify-center"><Download size={20} /></div><div><h2 className="font-display text-xl font-bold text-ink">Instalação do app</h2><p className="text-sm text-ink-2 mt-1">Defina quando o aviso de instalação reaparece para usuários móveis que ainda não instalaram o Crescer+.</p></div></div><div className="grid sm:grid-cols-2 gap-4">{PWA_SETTING_FIELDS.map((field) => <div key={field.key} className="rounded-2xl bg-[#FCFAF8] border border-[#F0E7E1] p-4"><Label htmlFor={field.key}>{field.label}</Label><p className="text-xs leading-relaxed text-ink-2 mt-1 mb-3">{field.description}</p><div className="flex gap-2"><Input id={field.key} type="number" min={field.min} max={field.max} value={settings[field.key] ?? ''} onChange={(e) => setSettings((current) => ({ ...current, [field.key]: e.target.value }))} className="h-11 rounded-xl bg-white border-[#EADFD8]" /><Button aria-label={`Salvar ${field.label}`} onClick={() => updateSetting(field, settings[field.key])} disabled={savingKey === field.key} className="h-11 rounded-xl bg-ink px-3"><Save size={16} /></Button></div></div>)}</div><div className="mt-4 rounded-2xl bg-[#FDF6F0] border border-[#F0E7E1] p-4 text-sm leading-relaxed text-ink-2">O aviso aparece somente na página Início, em dispositivos móveis, e deixa de ser exibido quando o navegador informa que o app já está instalado.</div></section>
